...
def gen_trials():
    ...
    _correct = [8, 81, 17, 72, 26, 63, 35, 54]
    #correct = [10, 10, 18, 18, 26, 26, 34, 34]
    #_correct = [10, 79, 18, 71, 26, 63, 34, 55]
    sim = [1, 1, 2, 2, 3, 3, 4, 4]
    simDict = dict(zip(_correct, sim))
    np.random.shuffle(correct)
    answerDict = dict(zip(chanLists, correct))

    nums1 = list(np.around(np.arange(0,1.01,1/44) * 100,0).astype(int))
...
def run_slider(Trials, trialTime, limitScale, subject, stem, numOptions, allTrials,
    ...
    stimLists = ['{}A'.format(i) for i in chanLists] + ['{}B'.format(i) for i in chanLists]
    chanCounter = dict(zip(stimLists, [1]*16))
    ...
    for i, (channel, shift, PM, AT) in enumerate(allTrials):
        ...
        if practise and i == 0:
            instructions = 'In this next task, a slider will appear at the bottom of the screen, with an image presented' \
                            ...
                            ' closely matches one that you saw in the first phase. The exact image that you saw will NOT be present. When you are satisfied, you can click' \
        ...
        if practise and i > 2:
            chanCounter['{}{}'.format(channel,PM)] += 1
  	elif not practise:
            chanCounter['{}{}'.format(channel,PM)] += 1
        ...
        while rawTimer.getTime() < trialTime and not submitted:  # kill trial if response locked, or time over
            ...
            synthim = '{}{}.png'.format(synthim.split('.png')[0], chanCounter['{}{}'.format(channel,PM)])
